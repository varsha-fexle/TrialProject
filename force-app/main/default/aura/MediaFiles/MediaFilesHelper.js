({
    ...
    //call on component load
    getSelectedObjLabel: function(component, event, helper, selectedSobjectAPIName) {
        
        if( selectedSobjectAPIName != 'Notes and Attachments' ) {
            var action = component.get("c.getSelectedObjLabel");
            
            action.setParams({"selectedObjAPIName"	: selectedSobjectAPIName});
            ...
            action.setCallback(this, function(response) {
                ...
                if (state === "SUCCESS") {
                    if(response.getReturnValue().isSuccess) {
                        ...
                        component.set('v.selectedSobjectLabel', response.getReturnValue().body);
                    }
                    else {
                        helper.showToast(component, event, helper, response.getReturnValue().status, response.getReturnValue().message);
                    }
            ...
            component.set('v.selectedSobjectLabel', selectedSobjectAPIName);
